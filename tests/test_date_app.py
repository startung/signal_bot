...
def test_no_args_no_default_returns_utc():
    app = make_app()
    response = next(app.handle("", sender="+440001111111"))
...
def test_city_returns_local_time(mock_tz):
    app = make_app()
    response = next(app.handle("London", sender="+440001111111"))
...
def test_city_and_country_returns_local_time(mock_tz):
    app = make_app()
    response = next(app.handle("New York, US", sender="+440001111111"))
...
def test_set_default_city(mock_tz):
    app = make_app()
    response = next(app.handle("set London, GB", sender="+440001111111"))
    assert "London" in response
...
def test_date_uses_default_after_set(mock_tz):
    app = make_app()
    next(app.handle("set Tokyo, JP", sender="+440001111111"))
    response = next(app.handle("", sender="+440001111111"))
...
def test_default_is_per_sender(mock_tz):
    app = make_app()
    next(app.handle("set Tokyo, JP", sender="+440001111111"))
    response = next(app.handle("", sender="+440002222222"))
    assert "UTC" in response
...
def test_invalid_city_returns_error(mock_tz):
    app = make_app()
    response = next(app.handle("Xyzzyville", sender="+440001111111"))
...
def test_set_default_persists_to_file(mock_tz, tmp_path):
    ...
    next(app.handle("set London, GB", sender="+440001111111"))
